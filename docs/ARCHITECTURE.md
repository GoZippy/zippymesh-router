...
ZippyMesh is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[ZippyMesh Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.zippymesh/db.json` when `DATA_DIR` is unset)
...
- main state: `${DATA_DIR}/db.json` (or `~/.zippymesh/db.json`)
- usage stats: `~/.zippymesh/usage.json`
- request log lines: `~/.zippymesh/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[ZippyMesh Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.zippymesh` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/zippymesh && npm run build`
- Build Docker image: `cd /root/dev/zippymesh && docker build -t zippymesh .`
