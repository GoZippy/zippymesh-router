...
export function openaiToClaudeRequest(model, body, stream) {
  ...
  if (body.messages && Array.isArray(body.messages)) {
    ...
    // CRITICAL: Claude requires each tool_result to have a matching tool_use in the *previous* message.
    // If the client sends tool messages before the assistant (wrong order), buffer tool_results and push after the next assistant.
    let currentRole = undefined;
    let currentParts = [];
    const pendingToolResults = [];

    const flushPendingToolResultsIfMatching = (assistantToolUseIds) => {
      if (assistantToolUseIds.length === 0 || pendingToolResults.length === 0) return;
      const idsSet = new Set(assistantToolUseIds);
      const matching = pendingToolResults.filter(b => idsSet.has(b.tool_use_id));
      if (matching.length === 0) return;
      result.messages.push({ role: "user", content: matching });
      for (let i = pendingToolResults.length - 1; i >= 0; i--) {
        if (idsSet.has(pendingToolResults[i].tool_use_id)) pendingToolResults.splice(i, 1);
      }
    };
    ...
    const flushCurrentMessage = () => {
      if (currentRole && currentParts.length > 0) {
        if (currentRole === "assistant") {
          const toolUseIds = currentParts.filter(b => b.type === "tool_use").map(b => b.id);
          result.messages.push({ role: currentRole, content: currentParts });
          currentParts = [];
          flushPendingToolResultsIfMatching(toolUseIds);
        } else {
          result.messages.push({ role: currentRole, content: currentParts });
          currentParts = [];
        }
    ...
    for (const msg of nonSystemMessages) {
      ...
      if (hasToolResult) {
        ...
        if (toolResultBlocks.length > 0) {
          // Accumulate consecutive tool_results into the same user message.
          // Claude requires ALL tool_results for a given assistant turn to be
          // in one user message — pushing them individually causes a 400 error.
          const isBatchingToolResults = currentRole === "user" &&
            currentParts.length > 0 &&
            currentParts.every(b => b.type === "tool_result");

          if (isBatchingToolResults) {
            // Already batching — just append to the current batch
            currentParts.push(...toolResultBlocks);
          } else {
            flushCurrentMessage();
            const lastMsg = result.messages[result.messages.length - 1];
            const lastIsAssistantWithToolUse = lastMsg?.role === "assistant" &&
              Array.isArray(lastMsg.content) &&
              lastMsg.content.some(b => b.type === "tool_use");
            const lastToolUseIds = lastIsAssistantWithToolUse
              ? (lastMsg.content.filter(b => b.type === "tool_use").map(b => b.id) || [])
              : [];
            if (lastIsAssistantWithToolUse && toolResultBlocks.every(tr => lastToolUseIds.includes(tr.tool_use_id))) {
              // Start batching into currentParts (flushed later as one user message)
              currentRole = "user";
              currentParts.push(...toolResultBlocks);
            } else {
              pendingToolResults.push(...toolResultBlocks);
            }
          }
        }
      ...
      if (hasToolUse) {
        flushCurrentMessage();
        const assistantToolUseIds = blocks.filter(b => b.type === "tool_use").map(b => b.id);
        flushPendingToolResultsIfMatching(assistantToolUseIds);
      }
    }
