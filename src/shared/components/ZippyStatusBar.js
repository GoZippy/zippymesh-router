...
export default function ZippyStatusBar() {
    ...
    useEffect(() => {
        // ── Node online/offline probe ─────────────────────────────────────────
        // Polls /api/node/status which makes a live HTTP probe to the sidecar
        // /health endpoint on every request. This avoids the HMR singleton bug
        // where the in-memory nodeProcess reference is lost on hot-reload.
        const checkNodeStatus = async () => {
            if (isDemoMode) return; // demo mode overrides below
            ...
            try {
                ...
                setIsConnected(false);
                setNodeError(err?.message || "Node status unavailable");
            }
        };

        // ── Full status + wallet poll ─────────────────────────────────────────
        ...
        // Probe online status immediately and every 5 s
        checkNodeStatus();
        const nodeInterval = setInterval(checkNodeStatus, 5000);

        // Fetch rich node info + wallet on a slower cycle
        checkStatus();
