...
function DashboardLayoutContent({ children, sidebarOpen, setSidebarOpen }) {
  ...
  const { mismatch, serverVersion, clientVersion, dismiss } = useVersionCheck();

  return (
    <div className="flex h-screen w-full overflow-hidden bg-bg">
      {/* Version mismatch banner — shown when server was upgraded but browser cache is stale */}
      {mismatch && (
        <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between gap-3 bg-amber-500 px-4 py-2 text-sm font-medium text-white shadow-md">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[16px]">update</span>
            <span>
              ZippyMesh was upgraded to v{serverVersion} — your browser has cached v{clientVersion}.
              Clear cache to get the latest UI.
            </span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => { window.location.reload(); }}
              className="rounded-md border border-white/40 px-3 py-1 text-xs font-semibold hover:bg-white/20 transition-colors"
            >
              Hard Reload
            </button>
            <button
              onClick={dismiss}
              className="rounded-md px-2 py-1 text-xs hover:bg-white/20 transition-colors"
              title="Dismiss"
            >
