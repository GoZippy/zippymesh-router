 ...
 * After each successful inference, settles a ZippyCoin micropayment.
 */
...
import { zippyRpc } from '@/lib/zippycoin-wallet';

// 1 microZIPc per token = 1_000_000_000_000 ZAT (10^12 ZAT / token)
const COST_PER_TOKEN_ZAT = 1_000_000_000_000n;
...
export async function POST(request) {
    try {
        ...
        // Calculate cost in ZAT based on actual tokens used.
        const tokensUsed = estimatedTokens;
        const costZat = BigInt(tokensUsed) * COST_PER_TOKEN_ZAT;
        const providerAddress = selectedProvider.wallet || selectedProvider.node_id;

        let paymentResult = { settled: false, txHash: null, cost_zat: costZat.toString() };
        try {
            const txHash = await zippyRpc('zippycoin_sendTransaction', [{
                from: wallet.address,
                to: providerAddress,
                value: '0x' + costZat.toString(16),
                data: '0x',
                nonce: 'auto'
            }]);
            paymentResult = { settled: true, txHash, cost_zat: costZat.toString() };
            console.log(`[Infer] Payment settled: ${costZat} ZAT → ${providerAddress} tx=${txHash}`);
        } catch (paymentErr) {
            // Payment failure MUST NOT block inference response — log only.
            console.warn('[Infer] Micropayment failed (inference still delivered):', paymentErr.message);
        }
        // ── End micropayment ───────────────────────────────────────────────────
