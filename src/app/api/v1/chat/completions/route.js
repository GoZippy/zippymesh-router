...
export async function POST(request) {
  ...
  try {
    ...
    if (useSmartRouting) {
      ...
      if (routing.success) {
        // Parallel execution: the response is already fetched — return it directly.
        if (routing.parallelExecution && routing.parallelResponse) {
          return new Response(routing.parallelResponse.body, {
            status: routing.parallelResponse.status,
            headers: routing.parallelResponse.headers,
          });
        }
