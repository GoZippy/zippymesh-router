...
// Check if config has ZippyMesh settings
const hasZippyMeshConfig = (config) => {
  if (!config) return false;
  return config.includes("model_provider = \"zippymesh\"") || config.includes("[model_providers.zippymesh]");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      config,
      hasZippyMesh: hasZippyMeshConfig(config),
...
// POST - Update ZippyMesh settings (merge with existing config)
export async function POST(request) {
  try {
    ...
    parsed._sections["model_providers.zippymesh"] = {
      name: "ZippyMesh",
...
export async function DELETE() {
  try {
    ...
    // Remove ZippyMesh related root fields only if they point to zippymesh
    if (parsed._root.model_provider === "zippymesh") {
