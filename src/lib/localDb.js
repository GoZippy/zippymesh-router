...
export function getSqliteDb() {
  ...
  sqliteDb.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_token_ledger_virtual_key ON token_ledger(virtual_key, ts);
    CREATE INDEX IF NOT EXISTS idx_token_ledger_provider    ON token_ledger(provider, model_id, ts);
  `);
...
// ── Vault meta (TOTP secret, backup codes, settings) ─────────────────────────

export function vaultMetaGet(key) {
  const db = getSqliteDb(); if (!db) return null;
  const row = db.prepare(`SELECT value FROM vault_meta WHERE key = ?`).get(key);
  return row ? row.value : null;
}

export function vaultMetaSet(key, value) {
  const db = getSqliteDb(); if (!db) return;
  db.prepare(`INSERT INTO vault_meta (key, value, updated_at) VALUES (?, ?, datetime('now'))
 ...
 * Written by the orchestrator on every successful completion.
 *
 * @param {Object} entry
 * @param {string}  entry.requestId
 * @param {string}  entry.provider
 * @param {string}  entry.modelId
 * @param {string}  [entry.virtualKey]   — virtual API key prefix/id used
 * @param {string}  [entry.clientId]     — x-zippy-client-id header
 ...
 */
export function recordTokenUsage({ requestId, provider, modelId, virtualKey, clientId, inputTokens, outputTokens, latencyMs, costUsd, status = 'success' }) {
  const db = getSqliteDb();
  if (!db) return;

  db.prepare(`
    INSERT INTO token_ledger
      (ts, request_id, provider, model_id, virtual_key, client_id,
       input_tokens, output_tokens, latency_ms, cost_usd, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    Date.now(),
    requestId || null,
...
/**
 * Query token usage aggregated by (provider, modelId) over a time window.
 * @param {number} [sinceMs]  — epoch ms lower bound (default: last 24h)
 * @returns {Array<{provider, modelId, inputTokens, outputTokens, requests, costUsd}>}
 */
export function getTokenUsageSummary(sinceMs) {
  const db = getSqliteDb();
  if (!db) return [];
...
/**
 * Query token usage for a specific virtual key.
 * @param {string} virtualKey
 * @param {number} [sinceMs]
 */
export function getTokenUsageByKey(virtualKey, sinceMs) {
  const db = getSqliteDb();
  if (!db) return [];

  const since = sinceMs ?? (Date.now() - 30 * 24 * 60 * 60 * 1000);  // 30 days
  return db.prepare(`
    SELECT
      provider, model_id AS modelId,
      COUNT(*)            AS requests,
      SUM(input_tokens)   AS inputTokens,
      SUM(output_tokens)  AS outputTokens,
      SUM(cost_usd)       AS costUsd
