...
export async function buildFailoverChain(context, originalProvider, originalModel, alreadyTried = [], config = null) {
    ...
    // Filter out degraded (provider, model) pairs — only skip if still in backoff window
    const sameModelAlternatives = (await findSameModelDifferentProvider(
        originalProvider,
        originalModel,
        availableConnections
    )).filter(c => !isModelDegraded(c.provider, c.model));
    ...
    for (const equiv of equivalentModels) {
        ...
        for (const conn of availableConnections) {
            ...
            if (matchingModel) {
                const modelId = typeof matchingModel === 'string' ? matchingModel : matchingModel.id;
                if (!isModelDegraded(conn.provider, modelId)) {
                    candidates.push({
                        connection: conn,
                        provider: conn.provider,
                        model: modelId,
