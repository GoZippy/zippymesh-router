...
async function handleSingleModelChat(body, modelStr, clientRawRequest = null, request = null, requestId = null) {
  ...
  // Virtual key prefix — first 12 chars of the bearer token for ledger bucketing
  const authHeader = request?.headers?.get("authorization") || "";
  const virtualKey = authHeader.startsWith("Bearer ")
    ? authHeader.slice(7, 19) || undefined
    : undefined;
