...
import { updateProviderConnection, getP2pSubscriptions, getPricingForModel, getNodeIdentity, addRateLimitSuggestion, getSettings, recordTokenUsage } from "@/lib/localDb.js";
...
import { recordModelFailure, recordModelSuccess } from "@/lib/modelHealth.js";
import { markModelUsed } from "@/lib/modelProber.js";
import { heartbeatCountRequest, heartbeatCountError } from "@/lib/mesh/heartbeat.js";
...
async function _executeOrchestratedChat(params) {
    ...
    for (const candidate of finalCandidates) {
        ...
        for (;;) {
            ...
            // Client errors (4xx) — track model health, then return terminal error
            if (result.status >= 400 && result.status < 500) {
                recordModelFailure(modelInfo.provider, modelInfo.model, String(result.status));
            ...
            if (result.success) {
                ...
                recordModelSuccess(modelInfo.provider, modelInfo.model);
                markModelUsed(modelInfo.provider, modelInfo.model);
                heartbeatCountRequest();
                break;
            }
        ...
        // Handle failure (record for circuit breaker + model health, then failover)
        circuitRecordFailure(modelInfo.provider);
        recordModelFailure(modelInfo.provider, modelInfo.model, String(result.status || 'error'));
        heartbeatCountError();
